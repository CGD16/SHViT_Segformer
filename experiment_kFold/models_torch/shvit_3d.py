...
class SqueezeExcite3D(nn.Module):
    ...
    Args:
        ...
        act_name (str, optional): Name of activation function. Default is 'relu'.
    ...
    def __init__(self, channels: int, rd_ratio: float=1/16, rd_channels: int=None, rd_divisor: int=8, add_maxpool: bool=False,
            bias: bool=True, norm_layer=None, act_name: str="relu"):
...
class PatchMerging3D(nn.Module):
    """
    The PatchMerging class implements a patch merging mechanism using three Conv3d_BN layers and an activation function.
    ...
    """
    def __init__(self, dim: int, out_dim: int, act_name: str="relu"):
        ...
        self.se = SqueezeExcite3D(channels=hid_dim, rd_ratio=0.25, act_name=act_name)
...
class Residual3D(nn.Module):
    ...
    def fuse(self):
        ...
        Returns:
            nn.Module: A fused convolutional layer if the main layer is Conv3d_BN. Otherwise, returns self.
        """
...
class FFN3D(nn.Module):
    """
    The FFN class implements a Feed-Forward Network with two Conv3d_BN layers (point-wise convolutions) and an activation.
    ...
    """    
    def __init__(self, ed: int, h: int, act_name: str="relu"):
...
class SHSA3D(nn.Module):
    ...
    """
    def __init__(self, dim: int, qk_dim: int, pdim: int, act_name: str="relu"):
        ...
        self.proj = nn.Sequential(act_cls(), Conv3d_BN(in_channels=dim, out_channels=dim, bn_weight_init=0.0))
...
class SHCSA3D(nn.Module):
    ...
    Outputs:
        x (torch.Tensor): Output tensor with the same shape as the input.        
    """
    def __init__(self, dim: int, qk_dim: int, pdim: int, kernel_size: int=3, act_name: str="relu"):
        ...
        self.qkv = Conv3d_BN(in_channels=pdim, out_channels=qk_dim*2 + pdim)
        self.proj = nn.Sequential(act_cls(), Conv3d_BN(in_channels=dim, out_channels=dim, bn_weight_init=0.0))
        self.center_att = CenterAttention3D(dim=pdim, kernel_size=kernel_size)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x1 = self.center_att(x1, D, H, W)        
        x1 = x1.reshape(B, D, H, W, self.pdim).permute(0,4,1,2,3)
...
class BasicBlock3D(nn.Module):
    ...
    Args:
        ...
        flg_is_last_stage (bool): center-attention used if not last stage
        use_center_att (bool): use center-attention (SHCSA3D) if flag is True, otherwise standard attention (SHSA3D)
		kernel_size (int, optional): Size of the convolutional kernel used in attention. Default is 3.
  		act_name (str, optional): Name of activation function. Default is 'relu'.
    ...
    def __init__(self, dim: int, qk_dim: int, pdim: int, block_type: str, flg_is_last_stage: bool, 
                 use_center_att: bool, kernel_size: int=3, act_name: str="relu"):
        ...
        if block_type == "s":  # for later stages
            ...
            if use_center_att and not flg_is_last_stage:
                self.mixer = Residual3D(SHCSA3D(dim=dim, qk_dim=qk_dim, pdim=pdim, kernel_size=kernel_size, act_name=act_name))
            else:
                self.mixer = Residual3D(SHSA3D(dim=dim, qk_dim=qk_dim, pdim=pdim, act_name=act_name))
            self.ffn = Residual3D(FFN3D(ed=dim, h=int(dim * 2), act_name=act_name))
...
class SHViT3D(nn.Module):
    ...
    Args:
        ...
        use_center_att (bool, optional): Whether to use center_attention in 2nd stage. Default is False.
        kernel_size (int, optional): Size of the convolutional kernel used in attention. Default is 3.
		act_name (str, optional): Name of activation function. Default is 'relu'.
    ...
    def __init__(self, in_channels: int=3, embed_dims=[224, 336, 448], partial_dims=[48, 72, 96],
                 ...
                 use_center_att: bool=False, kernel_size: int=3, act_name: str="relu"):
        ...
        for i in range(num_convs - 1):
            ...
            layers.append(Conv3d_BN(in_channels=in_channels, out_channels=out_channels, kernel_size=3, strides=2, padding=1))
            if i != num_convs - 2:  # Don't add activation function after the last Conv3d_BN
                layers.append(act_cls())
    ...
    def _build_stage(self):
        ...
        for i, (ed, kd, pd, dpth, do, t) in enumerate(zip(self.embed_dim, self.qk_dim, self.partial_dim, self.depth, self.down_ops, self.types)):
            ...
            for d in range(dpth):
                eval("blocks" + str(i+1)).append(BasicBlock3D(dim=ed, qk_dim=kd, pdim=pd, block_type=t, flg_is_last_stage=flg_is_last_stage, 
                                                              use_center_att=self.use_center_att, kernel_size=self.kernel_size, act_name=self.act_name))
            if do[0] == "subsample":
                ...
                blk.add_module("downsample1", nn.Sequential(
                    ...
                    Residual3D(FFN3D(ed=self.embed_dim[i], h=int(self.embed_dim[i] * 2), act_name=self.act_name)),
                ))
                blk.add_module("patch_merge", PatchMerging3D(dim=self.embed_dim[i], out_dim=self.embed_dim[i + 1], act_name=self.act_name))
                blk.add_module("downsample2", nn.Sequential(
                    ...
                    Residual3D(FFN3D(ed=self.embed_dim[i+1], h=int(self.embed_dim[i+1] * 2), act_name=self.act_name)),
                ))
