...
class Attention3D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, D: int, H: int, W: int) -> torch.Tensor:
        ...
        if self.sr_ratio > 1:
            x_reshaped = x.reshape(B, D, H, W, C).permute(0, 4, 1, 2, 3)  # (B, C, D, H, W)
        ...
        k = self.k(x_reshaped).reshape(B, -1, self.num_heads, self.head_dim).permute(0, 2, 3, 1)  # (B, num_heads, head_dim, N')
        v = self.v(x_reshaped).reshape(B, -1, self.num_heads, self.head_dim).permute(0, 2, 1, 3)  # (B, num_heads, N', head_dim)
